...
select
    ...
    umgppc.*
from [source].[ukgovt.minister_govuk_people_page_content_20260513] umgppc
where
...
select *
from [source].[ukgovt.minister_govuk_people_page_content_20260513] t1
where
    t1.[links.role_appointments.links.role.links.ordered_parent_organisations.title] = 'Cabinet Office Board' and
    not exists (
        select *
        from [source].[ukgovt.minister_govuk_people_page_content_20260513] t2
        where
