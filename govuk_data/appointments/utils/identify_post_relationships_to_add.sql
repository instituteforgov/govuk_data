...
-- NB: This uses GOV.UK's post_rank (where not null) as in a small number of cases we are fixing the post rank of an appointment
...
-- NB: This adds on the organisation short name, start and end dates so we can distinguish between organisations where the same name has been used more than once (e.g. Department for Culture, Media and Sport, Department for Education)
select distinct
...
where
    ...
    not exists (
        select *
        from core.post_relationship pr
